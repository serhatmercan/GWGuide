  METHOD /iwbep/if_mgw_appl_srv_runtime~create_stream.
    ...
    DATA lv_form_key    TYPE zmm_013_d_dms_tipi.
    DATA lv_material_no TYPE matnr.
    DATA lv_mimetype    TYPE nte_mimetype.

    IF iv_entity_name = 'DocumentSet'.
    ...
    IF line_exists( lt_return[ type = 'E' ] ).
      ...
      set_header( is_header = VALUE ihttpnvp( name  = 'Type'
                                              value = 'E'               ) ).
      set_header( is_header = VALUE ihttpnvp( name  = 'ID'
                                              value = ls_return-id      ) ).
      set_header( is_header = VALUE ihttpnvp( name  = 'Number'
                                              value = ls_return-number  ) ).
      set_header( is_header = VALUE ihttpnvp( name  = 'Message'
                                              value = ls_return-message ) ).
    ENDIF.
