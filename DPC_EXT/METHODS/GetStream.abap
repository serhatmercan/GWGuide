  ...
  METHOD /iwbep/if_mgw_appl_srv_runtime~get_stream.
    ...
    TRY.
      ...
      CATCH cx_fp_runtime_internal
            ...
            cx_fp_runtime_usage INTO DATA(lx_fpex). " TODO: variable is assigned but never used (ABAP cleaner)
    ENDTRY.

    ls_stream = VALUE ty_s_media_resource( mime_type = lv_mime_type
                                           value     = lv_content ).

    set_header( VALUE #( name  = 'Content-Disposition'
                         value = |{ lv_out_type }; filename="{ escape( val    = lv_file_name
