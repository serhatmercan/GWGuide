      ...
      WHEN 'Deep'.
        ...
        LOOP AT lt_select_options ASSIGNING FIELD-SYMBOL(<fs_select_option>).
          CASE <fs_select_option>-property.
            WHEN 'Key'.
              lo_filter->convert_select_option( EXPORTING is_select_option = <fs_select_option>
                                                IMPORTING et_select_option = lt_range_keys ).
  ...
  METHOD /iwbep/if_mgw_appl_srv_runtime~get_expanded_entity.
    CASE io_tech_request_context->get_entity_type_name( ).
      WHEN 'Header'.
        ...
        DATA lt_multi_deep      TYPE TABLE OF zcl_zsm_gw_001_mpc_ext=>ts_multi_deep.
        DATA lt_multi_item_deep TYPE TABLE OF zcl_zsm_gw_001_mpc_ext=>ts_multi_item_deep.

        lt_details = VALUE #( id = '01'
                              ( key = '001' type = 'ABC' message = 'Test I' )
                              ( key = '001' type = 'ABC' message = 'Test II' )
                              ( key = '001' type = 'DEF' message = 'Test III' )
                              ( key = '001' type = 'DEF' message = 'Test IV' )
                              ( key = '002' type = 'ABC' message = 'Test V' )
