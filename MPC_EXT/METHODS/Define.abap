  ...
  METHOD define.
    ...
    CONSTANTS lc_display_format TYPE /iwbep/med_annotation_key   VALUE 'display-format'.
    CONSTANTS lc_sap            TYPE /iwbep/med_anno_namespace   VALUE 'sap'.
...
*  CONSTANTS lc_aggregate            TYPE /iwbep/med_annotation_value VALUE 'aggregate'.
    CONSTANTS lc_applicable_path      TYPE /iwbep/med_annotation_key   VALUE 'applicable-path'.
    CONSTANTS lc_date                 TYPE /iwbep/med_annotation_value VALUE 'Date'.
    CONSTANTS lc_descendant_count_for TYPE /iwbep/med_annotation_key   VALUE 'hierarchy-node-descendant-count-for'.
    CONSTANTS lc_display_format       TYPE /iwbep/med_annotation_key   VALUE 'display-format'.
    CONSTANTS lc_drill_state_for      TYPE /iwbep/med_annotation_key   VALUE 'hierarchy-drill-state-for'.
    CONSTANTS lc_filter_restriction   TYPE /iwbep/med_annotation_key   VALUE 'filter-restriction'.
    CONSTANTS lc_email                TYPE /iwbep/med_annotation_value VALUE 'email'.
    CONSTANTS lc_fixed_values         TYPE /iwbep/med_annotation_value VALUE 'fixed-values'.
    CONSTANTS lc_interval             TYPE /iwbep/med_annotation_value VALUE 'interval'.
    CONSTANTS lc_multi_value          TYPE /iwbep/med_annotation_value VALUE 'multi-value'.
    CONSTANTS lc_node_for             TYPE /iwbep/med_annotation_key   VALUE 'hierarchy-node-for'.
    CONSTANTS lc_non_negative         TYPE /iwbep/med_annotation_value VALUE 'NonNegative'.
    CONSTANTS lc_label                TYPE /iwbep/med_annotation_key   VALUE 'label'.
    CONSTANTS lc_level_for            TYPE /iwbep/med_annotation_key   VALUE 'hierarchy-level-for'.
    CONSTANTS lc_parent_node_for      TYPE /iwbep/med_annotation_key   VALUE 'hierarchy-parent-node-for'.
    CONSTANTS lc_required_in_filter   TYPE /iwbep/med_annotation_key   VALUE 'required-in-filter'.
    CONSTANTS lc_sap                  TYPE /iwbep/med_anno_namespace   VALUE 'sap'.
    CONSTANTS lc_semantics            TYPE /iwbep/med_annotation_key   VALUE 'semantics'.
    CONSTANTS lc_single_value         TYPE /iwbep/med_annotation_value VALUE 'single-value'.
    CONSTANTS lc_text                 TYPE /iwbep/med_annotation_key   VALUE 'text'.
    CONSTANTS lc_true                 TYPE /iwbep/med_annotation_value VALUE 'true'.
    CONSTANTS lc_unit                 TYPE /iwbep/med_annotation_key   VALUE 'unit'.
    CONSTANTS lc_upper_case           TYPE /iwbep/med_annotation_value VALUE 'UpperCase'.
    ...
    TRY.
        ...
        " Set as E-Mail
        lo_annotation->add( iv_key   = lc_semantics
                            iv_value = lc_email ).

        " Set as Text (Key & Description)
        lo_annotation->add( iv_key   = lc_text
                            iv_value = 'Description' ).

        " Set as Unit (Amount & Unit)
        lo_annotation->add( iv_key   = lc_unit
                            iv_value = 'Unit' ).
        ...
        " Set Filter Interval
        lo_annotation->add( iv_key   = lc_filter_restriction
                            iv_value = lc_interval ).
        ...
        lo_annotation->add( iv_key   = lc_filter_restriction
                            iv_value = lc_multi_value ).

        " Set Filter Single
        lo_annotation->add( iv_key   = lc_filter_restriction
                            iv_value = lc_single_value ).

        " Set Function Import Triggable
        lo_action->/iwbep/if_mgw_odata_annotatabl~create_annotation( lc_sap )->add( iv_key   = lc_applicable_path
                                                                                    iv_value = 'ID' ).
        ...
        IF lo_entity IS BOUND.
          TRY.
              ...
              lo_annotation->add( iv_key   = lc_node_for
                                  iv_value = 'NodeID' ).
            CATCH /iwbep/cx_mgw_med_exception.
          ENDTRY.
          ...
          TRY.
              ...
              lo_annotation = lv_property->/iwbep/if_mgw_odata_annotatabl~create_annotation( lc_sap ).
              lo_annotation->add( iv_key   = lc_level_for
                                  iv_value = 'NodeID' ).
            CATCH /iwbep/cx_mgw_med_exception.
          ENDTRY.
          ...
          TRY.
              ...
              lo_annotation = lv_property->/iwbep/if_mgw_odata_annotatabl~create_annotation( lc_sap ).
              lo_annotation->add( iv_key   = lc_parent_node_for
                                  iv_value = 'NodeID' ).
            CATCH /iwbep/cx_mgw_med_exception.
          ENDTRY.
          ...
          TRY.
              ...
              lo_annotation->add( iv_key   = lc_drill_state_for
                                  iv_value = 'NodeID' ).
            CATCH /iwbep/cx_mgw_med_exception.
          ENDTRY.
          ...
          TRY.
              ...
              lo_annotation = lo_property->/iwbep/if_mgw_odata_annotatabl~create_annotation( lc_sap ).
              lo_annotation->add( iv_key   = lc_descendant_count_for
                                  iv_value = 'NodeID' ).
            CATCH /iwbep/cx_mgw_med_exception.
          ENDTRY.
        ENDIF.
      ...
      CATCH /iwbep/cx_mgw_med_exception INTO DATA(lo_exception). " TODO: variable is assigned but never used (ABAP cleaner)
    ENDTRY.
  ENDMETHOD.
  ...
  METHOD define.
    ...
    lo_mpc_util = NEW #( io_model        = model
                         io_va_model     = vocab_anno_model
                         iv_service_name = 'ZSM_ODATA_TEST_SRV' ).

    super->define( ).

    lo_mpc_util->add_value_help( iv_annotation_target = '/SM/ZSM_ODATA_TEST_SRV.Header/Key'
    ...
    lo_mpc_util->set_display_date( iv_entity_name = 'Header'
                                   iv_property    = 'Key' ).
    lo_mpc_util->set_display_non_negative( iv_entity_name = 'Header'
                                           iv_property    = 'Key' ).
    lo_mpc_util->set_display_upper_case( iv_entity_name = 'Header'
                                         iv_property    = 'Key' ).
    ...
    lo_mpc_util->set_filterable( iv_entity_name = 'Header'
                                 iv_property    = 'Key' ).
    lo_mpc_util->set_filter_interval( iv_entity_name = 'Header'
                                      iv_property    = 'Key' ).
    lo_mpc_util->set_filter_mandatory( iv_entity_name = 'Header'
                                       iv_property    = 'Key' ).
    lo_mpc_util->set_filter_multi_value( iv_entity_name = 'Header'
                                         iv_property    = 'Key' ).
    lo_mpc_util->set_filter_single( iv_entity_name = 'Header'
                                    iv_property    = 'Key' ).
    lo_mpc_util->set_function_import_triggable( iv_action_name = 'GetPersonelInformation'
                                                iv_property    = 'ID' ).
    ...
    lo_mpc_util->set_name( iv_entity_name = 'Header'
                           iv_property    = 'Key' ).
